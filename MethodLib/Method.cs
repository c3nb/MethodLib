namespace MethodLib
{
    ...
    public class Method
    {
        ...
        public readonly ReadOnlyCollection<Instruction> Instructions;
        ...
        public IntPtr FunctionPointer
        {
            get
            {
                if (fPtr != IntPtr.Zero)
                    return fPtr;
                return fPtr = MethodHandle.GetFunctionPointer();
            }
        }
        public IntPtr HandleValue
        {
            get
            {
                if (hVal != IntPtr.Zero)
                    return hVal;
                return hVal = MethodHandle.Value;
            }
        }
        private IntPtr fPtr;
        private IntPtr hVal;
        Method(DynamicMethod method, Method orig)
        {
            Base = method;
            Body = orig.Body;
            MethodGenerics = orig.MethodGenerics;
            TypeGenerics = orig.TypeGenerics;
            LocalVars = orig.LocalVars;
            Module = method.Module;
            Buffer = orig.Buffer;
            insts = orig.insts;
            Instructions = orig.Instructions;
            ExceptionHandlers = orig.ExceptionHandlers;
        ...
        public Method(MethodBase method)
        {
            ...
            insts = new List<Instruction>((bytes.Length + 1) / 2);
            ...
            ReturnType = Base is MethodInfo mi ? mi.ReturnType : typeof(void);
            ...
            {
                Instruction.FindInstruction(insts, ex.TryOffset)
                    .Block = new ExceptionBlock(beginBlock);
                Instruction.FindInstruction(insts, ex.TryLength + ex.HandlerLength - 1).Next
                    .Block = new ExceptionBlock(endBlock);
                switch (ex.Flags)
                {
                    case ExceptionHandlingClauseOptions.Filter:
                        Instruction.FindInstruction(insts, ex.FilterOffset)
                            .Block = new ExceptionBlock(beginFilter);
                        break;
                    case ExceptionHandlingClauseOptions.Finally:
                        Instruction.FindInstruction(insts, ex.HandlerOffset)
        ...
        private readonly List<MethodInfo> prefixes = new List<MethodInfo>();
        private readonly List<MethodInfo> postfixes = new List<MethodInfo>();
        private readonly ByteBuffer Buffer;
        private readonly List<Instruction> insts;
        private readonly MethodBody Body;
        private Method copy;
        public Method Copy()
        {
            if (copy != null) return copy;
            DynamicMethod dm = new DynamicMethod($"{Base.Name}_Copy", ReturnType, ParameterTypes);
            Copy(dm.GetILGenerator());
            var handle = Compile(dm);
            copy = new Method(dm, this);
            copy.MethodHandle = handle;
            return copy;
        }
        private void Copy(ILGenerator il)
        {
            PrepareLabels(il);
            PrepareLocals(il);
        ...
        public void AddPrefix(MethodInfo prefix, bool updateWrapper = false)
        {
            prefixes.Add(prefix);
            if (updateWrapper)
                UpdateWrapper();
        }
        public MethodInfo AddPrefix(Delegate prefix, bool updateWrapper = false)
        {
            MethodInfo wrapper;
        ...
        public void AddPostfix(MethodInfo postfix, bool updateWrapper = false)
        {
            postfixes.Add(postfix);
            if (updateWrapper)
                UpdateWrapper();
        }
        public MethodInfo AddPostfix(Delegate postfix, bool updateWrapper = false)
        {
            MethodInfo wrapper;
        ...
        public void RemovePrefix(MethodInfo prefix, bool updateWrapper = false)
        {
            prefixes.Remove(prefix);
            if (updateWrapper)
                UpdateWrapper();
        }
        public void RemovePostfix(MethodInfo postfix, bool updateWrapper = false)
        {
        ...
        {
            Replace(this, MakeFixedMethod(option));
            Attached = true;
        }
        public void Detach()
        {
            Recover(FunctionPointer);
            Attached = false;
        }
        ...
        private Method MakeFixedMethod(FixOption option)
        {
            ...
            il.Emit(OpCodes.Ret);
            var handle = Compile(method);
            var meth = new Method(method, this);
            meth.MethodHandle = handle;
            return meth;
        }
        ...
        public static implicit operator Method(MethodInfo method) => GetMethod(method);
        public static implicit operator Method(ConstructorInfo method) => GetMethod(method);
        ...
        public static implicit operator DynamicMethod(Method method) => method.Base as DynamicMethod;
        #region Utils
        ...
        static readonly Dictionary<IntPtr, List<ushort>> Cache = new Dictionary<IntPtr, List<ushort>>();
        public static unsafe void TryNoInlining(Method method)
        {
            if (IsMonoRuntime)
                *((ushort*)method.HandleValue + 1) |= 8;
        }
        public static unsafe void Replace(Method target, Method method)
        {
            TryNoInlining(target);
            IntPtr targetHandle = target.FunctionPointer, methodHandle = method.FunctionPointer;
            if (Environment.OSVersion.Platform < PlatformID.Unix)
                VirtualProtect(targetHandle, new IntPtr(1), 0x40, out _);
            byte* src = (byte*)targetHandle;
            ...
            if (IntPtr.Size == sizeof(long))
            {
                ...
                cache.Add(*(ushort*)src);
                src = Write(src, methodHandle.ToInt64());
                cache.Add(*(ushort*)src);
            ...
            else
            {
                ...
                cache.Add(*src);
                src = Write(src, methodHandle.ToInt32());
                cache.Add(*src);
                Write(src, (byte)0xC3);
            }
            if (!Cache.ContainsKey(targetHandle))
                Cache.Add(targetHandle, cache);
        }
        public static unsafe void Recover(IntPtr target)
        {
